...
class Komentator {
    ...
    public void relacjonujZadanie(int nrElementu, Uczestnik u, DziedzinaZadania zd)
    {
        String slowo_uczestnik = odmienUczestnik(u);
        slowo_uczestnik = slowo_uczestnik.substring(0, 1).toUpperCase() + slowo_uczestnik.substring(1);
        mow( slowo_uczestnik+" "+u+" robi teraz zadanie z dziedziny : "+zd+". Element nr "+nrElementu+" trasy.");
    }

    public void KomentujKoniec(Iterable<Uczestnik> uczestnicy, List<Uczestnik> meta)
    {
        mow("PROSZĘ PAŃSTWA MAMY ZWYCIĘZCE, PIERWSZY NA METĘ DOTARŁ ");

        for (Uczestnik u : meta)
        {
